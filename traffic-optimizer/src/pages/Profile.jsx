...
export default function Profile() {
  ...
  const handleSaveName = async () => {
    ...
    try {
      ...
      await axios.put(
        "${import.meta.env.VITE_API_URL}/auth/profile/name",
        { name: newName.trim() },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const modeConfig =
    MODE_CONFIG[profile?.favourite_mode] || MODE_CONFIG.None;
    ...
    >
      ...
      >
        ...
        >
          {[
            {
              number: profile?.total_routes,
              label: "Total Routes",
              color: "#10B981",
            },
            {
              number: modeConfig.icon,
              label: "Fav Mode",
              sub: modeConfig.label,
              color: modeConfig.color,
            },
            {
              number: profile?.recent_routes?.length || 0,
              label: "Recent",
              color: "#3B82F6",
            },
          ].map((stat, i) => (
